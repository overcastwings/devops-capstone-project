...
def list_accounts():
    ...
    """
    app.logger.info("Requesting a list of all accounts")
...
def read_account(id):
    """
    Reads account, given an ID.
    """
    app.logger.info("Request to read an Account with id: %s", id)
    account = Account.find(id)
    if not account:
        abort(status.HTTP_404_NOT_FOUND, f"Unable to locate acct. with id={id}")
    else:
...
def update_accounts(id):
    """
    Update an account based on posted data...
    """
